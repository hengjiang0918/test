			...
			<div class="exam_con">
				...
				<table  cellspacing="0" border="1" class="kaoshi_table">
					<tr height="30">
						...
						<td>当前${pager.cur_page}/总共${pager.pageCount}页</td>
				...
				<table  cellspacing="0" border="1" class="kaoshi_table">
				  ...
				  <c:forEach items="${carList}" var="d">
					  <tr>
					    ...
					    <td>${d.carFee }</td>
					    <td>
					    <c:if test="${d.financeType==1}">包月停车</c:if>
					    <c:if test="${d.financeType==2}">临时停车</c:if>
					    </td>
					    <td>
					    <c:if test=""></c:if>
					    <c:if test="${d.status==0}">未出场</c:if>
					    <c:if test="${d.status==1}">已出场</c:if>
					    <c:if test="${d.status==2}">已出场</c:if>
					    </td>
					    <td>${d.userName}</td>
